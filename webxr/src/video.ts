export type VideoStats = {
  state: string;
  streams: Array<{ id: string; fps: number; bitrateKbps: number; width: number; height: number; packetsLost: number; jitter: number }>;
};
...
export class VideoClient {
  ...
  private async handleMessage(msg: any) {
    if (msg.type === "video_offer") {
      ...
      this.ws.send(JSON.stringify({ type: "video_answer", data: this.pc.localDescription }));
      this.startStats();
    }
    ...
    this.statsTimer = window.setInterval(async () => {
      ...
      report.forEach((stat) => {
        if (stat.type === "inbound-rtp" && stat.kind === "video") {
          const bitrateKbps = stat.bytesReceived && stat.timestamp ? Math.round((stat.bytesReceived * 8) / 1000) : 0;
          streams.push({
